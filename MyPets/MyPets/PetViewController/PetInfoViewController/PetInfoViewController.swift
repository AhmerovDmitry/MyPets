        ...
        didSet {
            if showEditedButtons {
                UIView.animate(withDuration: 0.5, animations: {
                    ...
                    self.cameraButton.frame.origin = CGPoint(
                        x: self.rightBarButtonFrame.origin.x,
                        y: self.rightBarButtonFrame.origin.y + self.rightBarButtonFrame.height * 1.25
                    )
                    self.editedButton.frame.origin = CGPoint(
                        x: self.rightBarButtonFrame.origin.x,
                        y: self.cameraButton.frame.origin.y + self.rightBarButtonFrame.height * 1.25
                    )
                    self.deleteButton.frame.origin = CGPoint(
                        x: self.rightBarButtonFrame.origin.x,
...
//    ]
    ...
    let collectionView: UICollectionView = {
        ...
        collectionView.register(PetViewCollectionCell.self, forCellWithReuseIdentifier: "collectionCellPetId")
...
//        }
    ...
    func setupElements() {
        [titleImage,
         backgroundView,
         picker,
         saveDateButton].forEach { $0.translatesAutoresizingMaskIntoConstraints = false }
